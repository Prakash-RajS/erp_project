...
class Role(models.Model):
    role = models.CharField(max_length=100, unique=True)  # Role name
    description = models.TextField(blank=True, null=True)
    
    department = models.ForeignKey(
        Department, on_delete=models.CASCADE, related_name='roles'
    )
    department_name = models.CharField(max_length=100, blank=True)  # auto-filled

    branch = models.ForeignKey(
        Branch, on_delete=models.SET_NULL, null=True, blank=True
    )
    branch_name = models.CharField(max_length=100, blank=True)  # auto-filled
...
# Existing models (if any) remain unchanged
